...
- Region creation is manual. Do not restore hover-following window outlines,
  automatic window snapping, or click-to-select-window behavior. Users drag a
  region, move it, and resize it with eight handles.
...
- `CaptureCoordinator` obtains permission and freezes the active display. It
  does not collect visible-window geometry for selection.
...
- Verify manual region drag, move, and all eight resize handles.
