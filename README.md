...
- 也可保存、贴图或进入完整编辑器继续处理
- 框选区域后可从“更多”启动无音频录屏，停止后自动保存为 MP4
...
> kiri 目前处于早期源码预览阶段。区域录屏与短视频 GIF 已进入 v0.2 预览；
> 系统音频、麦克风、暂停、裁剪和滚动长截图仍在开发中。
...
1. 按 **⇧⌘A** 并框选录制区域。
2. 点击工具栏“更多”，选择 **Record Region**。
3. 从菜单栏 Kiri 或“Capture”菜单停止录制，MP4 会自动进入素材库。
4. 对 15 秒以内的视频打开“更多”，选择 **Convert to GIF**。

首个预览版录制鼠标指针但不录音；音频、暂停和裁剪会在后续 v0.2 迭代加入。

## 接下来

- **v0.1**：实体多显示器验收、正式签名与 Apple 公证
- **v0.2**：完善录屏状态浮层、系统/麦克风音频、暂停继续和裁剪
