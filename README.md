...
- 录制时悬浮显示计时、暂停/继续和停止；暂停时间不会进入最终视频
...
1. 按 **⇧⌘A** 并框选录制区域。
2. 在底部第一层模式选择器中切换到 **Record**，再框选录制区域。
3. 自动出现录屏设置，按需切换倒计时、系统声音、麦克风、鼠标指针和点击高亮。
4. 点击 **Start Recording**；3·2·1 期间可按 Esc 取消。
5. 使用悬浮控制条暂停、继续或停止；菜单栏 Kiri 与“Capture”菜单提供相同操作。
6. 停止后 MP4 自动进入素材库；15 秒以内的视频还能选择 **Convert to GIF**。
