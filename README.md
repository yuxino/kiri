...
- 选区浮层第一层支持滚动长截图；先固定区域，再逐段滚动、截取、撤销或完成并复制
...
2. 在原应用中滚动；点击 **截取下一段**，或在控制面板中按 **Space / Return**。
3. 需要时点击 **撤销上一段**；完成后点击 **完成并复制**。按 **Esc** 可取消。
4. Kiri 会自动拼接各段，复制最终 PNG，并将同一份滚动长截图保存到本地素材库。

控制面板会在每次截取前隐藏，因此不会出现在下一段画面中。请保持指针位于最初选择的显示器；当前版本提供引导式拼接，手动接缝调整仍在开发中。
