...
- 也可保存、贴图或进入完整编辑器继续处理
- 工具栏直接提供区域录屏；开始前可切换 3 秒倒计时、系统声音、麦克风、鼠标指针和点击高亮
- 录制前显示可按 Esc 取消的 3·2·1，停止后自动保存为 H.264 MP4
...
1. 按 **⇧⌘A** 并框选录制区域。
2. 点击工具栏直接显示的 **Record Region**，按需切换倒计时、系统声音、麦克风、鼠标指针和点击高亮。
3. 点击 **Start Recording**；3·2·1 期间可按 Esc 取消。
4. 从菜单栏 Kiri 或“Capture”菜单停止录制，MP4 会自动进入素材库。
5. 对 15 秒以内的视频打开“更多”，选择 **Convert to GIF**。

音频默认关闭并记忆上次选择；只有打开麦克风时才会请求麦克风权限。麦克风和系统原生点击高亮需要 macOS 15 或更高版本。

## 接下来

- **v0.1**：实体多显示器验收、正式签名与 Apple 公证
- **v0.2**：完善录屏状态浮层、暂停继续和裁剪
