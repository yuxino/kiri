...
当前图标使用白色圆角底、灰紫线稿和紫色取景框，和 macOS Dock 中的轻量应用图标保持同一视觉节奏；GitHub README 与 macOS App 使用同一份图标资源。
...
- 冻结当前屏幕后用单层紫色轮廓预览并单击选择窗体，或手动拖出精确选区；两种选区都可移动和八向拉伸
...
- 选区浮层第一层支持滚动长截图；固定区域后只需慢慢滚动，Kiri 会自动跟随截取并实时预览
...
1. 按 **⇧⌘A**，在第一层模式选择器中切换到 **滚动长截图**，然后框选区域。
2. 点击 **开始截取**，回到原应用慢慢向下滚动；Kiri 会自动识别重叠区域并持续补充预览。
3. 滚动结束后点击 **停止截取**；需要时可撤销上一段，再点击 **完成并复制**。按 **Esc** 可停止或取消。
4. Kiri 会自动拼接各段，复制最终 PNG，并将同一份滚动长截图保存到本地素材库。

控制面板始终悬浮在旁边，但会从屏幕采集中排除，因此不会进入最终图片。建议每次滚动保留一部分重叠；如果跨得太远，Kiri 会提示你稍微滚回。当前版本会沿用采集时确认的接缝，避免导出时重新识别造成整段重复；手动接缝调整仍在开发中。

## 接下来

- **v0.1**：实体多显示器验收、正式签名与 Apple 公证
- **v0.2**：完善录屏状态浮层和裁剪
- **v0.3**：滚动长截图和自动拼接已支持；接缝修正与超长页面的内存优化仍在规划中
