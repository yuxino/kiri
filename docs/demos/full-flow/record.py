"""One continuous actual-frontend session; isolated native boundaries, no external network or keys."""
import argparse,asyncio,base64,hashlib,io,json,os,re,subprocess,threading,time
...
async def run(args):
 ...
 report={'success':False,'source_tree':'fcd30ff9d323dd79f5adf248bbd3a82070d262ce','source_commit':'cea661eff7d05ce63210825f9f0014ca494f67a5','artifact_id':9985893052,'artifact_run':34022248971,'native':False,'single_take':True,'scene_cuts':0,'speed_factor':10,'scenes':[],'checks':{},'errors':[]}
 async with async_playwright() as pw:
  browser=await pw.chromium.launch(executable_path=os.environ.get('CHROME_BIN','/usr/bin/google-chrome'),args=['--disable-dev-shm-usage'])
  context=await browser.new_context(viewport={'width':1280,'height':720},device_scale_factor=1.5,locale='zh-CN',color_scheme='light')
  await context.route('**/*',lambda r:r.continue_() if r.request.url.startswith('http://127.0.0.1:8791/') else r.abort())
  await context.add_init_script(script=(ROOT/'bridge.js').read_text())
  page=await context.new_page();page.set_default_timeout(12000)
  page.on('pageerror',lambda e:report['errors'].append(str(e)))
  subject_task=None;subject_frames=[];capturing=False;paused=False;clip=None
  async def record_subject():
   while capturing:
    if not paused:
     try:subject_frames.append((time.monotonic(),await page.screenshot(type='png',clip=clip,animations='allow')))
     except Exception:break
    await asyncio.sleep(.3)
  async def backend(source,cmd,a):
   nonlocal capturing,paused,clip,subject_task,subject_frames
   if cmd=='freeze':
    MEDIA['frozen']=(await page.frame_locator('#subject').locator('body').screenshot(type='png'),'image/png');return None
   if cmd=='save_png':
    data=bytes(a['bytes']);Image.open(io.BytesIO(data)).verify()
    MEDIA[a['id']]=(data,'image/png');MEDIA['thumb-'+a['id']]=(data,'image/png');return None
   if cmd=='record_start':
   ...
   if cmd=='record_stop':
    ...
    if len(subject_frames)<4:raise RuntimeError('Insufficient real subject frames')
    ...
    MEDIA[a['id']]=(path.read_bytes(),'video/mp4');MEDIA['thumb-'+a['id']]=(subject_frames[-1][1],'image/png')
    report['checks']['real_browser_subject_frames']=len(subject_frames);return None
  ...
  await page.screenshot(path=str(OUT/'initial.png'));lib=page.frame_locator('#library')
  if args.inspect:
  ...
  start=time.monotonic();frames=[];cdp=await context.new_cdp_session(page)
  folder=OUT/'frames';folder.mkdir(exist_ok=True)
  async def receive(event):
   try:
    data=base64.b64decode(event['data']);fname=f'{len(frames):06d}.jpg'
    (folder/fname).write_bytes(data);frames.append({'file':fname,'t':event['metadata']['timestamp'],'size':len(data)})
   finally:await cdp.send('Page.screencastFrameAck',{'sessionId':event['sessionId']})
  ...
  async def mark(name,delay=8):
   await wait(delay);stamp=time.monotonic()-start;report['scenes'].append({'name':name,'t':stamp});await page.screenshot(path=str(OUT/f'proof-{len(report["scenes"]):02d}.png'));print(name,round(stamp,1),flush=True)
  ...
  async def capture(mode):
   await page.evaluate('launchCapture()');o=page.frame_locator('#overlay');await click(o.get_by_role('button',name=mode,exact=True));return o
  try:
   await mark('素材库')
   await click(lib.get_by_role('button',name='设置',exact=True));await mark('设置')
   ...
   await click(o.get_by_title('矩形 (R)',exact=True));await drag(236,174,882,243)
   ...
   await click(o.get_by_role('button',name=re.compile('完成.*Return')));await mark('截图完成与本地素材',4)
   o=await capture('文字');await drag(210,150,1015,292)
   await o.get_by_text('识别结果',exact=True).wait_for();await mark('文字识别结果')
   ...
   await page.screenshot(path=str(OUT/'countdown-cancel.png'));await click(c.get_by_role('button',name='取消倒计时'),pause=0)
   ...
   for n in [3,2,1]:
    await c.get_by_role('status',name=re.compile(str(n))).wait_for();await page.screenshot(path=str(OUT/f'countdown-{n}.png'));seen.append(n)
   report['checks']['countdown_numerals']=seen
   ctrl=page.frame_locator('#control-panel');await ctrl.get_by_title('暂停录制',exact=True).wait_for();await mark('黑色倒计时结束，录制控制条出现')
   ...
   report['checks']['saved_assets']=await page.evaluate('state.assets.map(x=>x.kind)');report['success']=True
  except Exception as e:
   report['failure']=str(e);print('FAIL',str(e),flush=True);await page.screenshot(path=str(OUT/'failure.png'))
   report['ui']=[{'url':f.url,'text':await f.evaluate('document.body.innerText'),'controls':await f.evaluate("[...document.querySelectorAll('button,textarea,input')].map(x=>({tag:x.tagName,text:x.innerText,title:x.title,aria:x.getAttribute('aria-label'),placeholder:x.getAttribute('placeholder')}))")} for f in page.frames]
  await cdp.send('Page.stopScreencast');await asyncio.sleep(.3)
  report['raw_seconds']=time.monotonic()-start;report['calls']=await page.evaluate('state.calls');report['frames']=frames
