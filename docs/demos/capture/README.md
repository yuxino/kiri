...
To reproduce a scene, build each project at the source commit recorded in its `docs/demos/provenance.json`, then place its `dist` output in `dist/demo-frontend-<project>/` next to these scripts and write that commit to `source-commit.txt`. Projects: kiri, mimi, satori, viva, tick.

Only the listed public projects belong in this recorder and its generated documentation. Do not add private project names, addresses, assets, or examples to public demos.
