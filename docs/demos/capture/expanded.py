...
async def record(repo,browser):
    ...
    await context.add_init_script(script=(ROOT/'bridge.js').read_text().replace('__PROJECT__',repo)+"\nif (window.__TAURI_INTERNALS__) globalThis.isTauri=true;")
    ...
    try:
        ...
        if repo=='kiri':
            ...
            await click(f.get_by_label('Line',exact=True));await f.get_by_label('Line',exact=True).press('ArrowRight');await f.get_by_label('Line',exact=True).press('ArrowRight');await mark('调整线宽，不必重新画','02 / Adjust the annotation style')
            await click(f.get_by_title('Arrow (A)'));await drag(.69,.59,.55,.43);await mark('箭头：把视线引向关键细节','03 / Draw an arrow')
            ...
            await click(f.get_by_title('Mosaic (M)'));await click(f.get_by_title('Strong',exact=True));await drag(.135,.875,.50,.875);await mark('马赛克：遮住不想展示的区域','06 / Pixel mosaic with adjustable strength')
            await click(f.get_by_title('Gaussian blur',exact=True));await drag(.57,.875,.83,.875);await mark('也可以换成柔和的模糊','07 / Switch to the blur brush')
            ...
            await f.get_by_label('Interface Language',exact=True).select_option('zh');actions.append('Switch UI language to Chinese');await mark('中英文界面，一键切换','10 / Switch the interface language')
        elif repo=='viva':
            ...
            await click(f.get_by_role('tab',name='Weekend.md',exact=True));await click(f.get_by_label('Source',exact=True));await f.get_by_role('textbox',name='Editing Weekend.md',exact=True).click();await page.keyboard.press('Control+f');await type_in(f.get_by_label('Find in document',exact=True),'small');await mark('在当前文档里查找关键词','08 / Find in the current document')
            ...
            await click(f.get_by_label('Dark',exact=True));await mark('也提供深色工作区','11 / Dark appearance')
            await click(f.get_by_label('Light',exact=True));await click(f.get_by_label('Return to document',exact=True));await mark('回到浅色，接着写','12 / Return to the document')
        elif repo=='satori':
            ...
            await click(f.get_by_role('button',name='添加变量',exact=True));await type_in(f.get_by_placeholder('变量名'),'DEMO_MODE');await type_in(f.get_by_placeholder('变量值'),'preview');await mark('给脚本配置自己的环境变量','09 / Configure non-secret example variables')
            ...
            await click(f.get_by_label('放大阅读页面',exact=True));await click(f.get_by_label('放大阅读页面',exact=True));await mark('放大局部，细节看得更清楚','07 / Zoom into the artwork')
