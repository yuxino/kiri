...
async def record(repo,browser):
    ...
    try:
        ...
        if repo=='kiri':
            ...
            await page.keyboard.press('Escape');await page.wait_for_timeout(1500);await shot('poster')
            ...
            await click(frame.get_by_role('button',name='Source',exact=True))
            editor=frame.get_by_role('textbox',name='Markdown editor',exact=True)
            ...
            await click(frame.get_by_role('button',name='Split',exact=True));await shot('poster');await page.wait_for_timeout(1600)
            await caption('切回 Live，把注意力留给文字。','Switch to Live and keep the focus on your words.')
            await click(frame.get_by_role('button',name='Live',exact=True));await shot('02');await page.wait_for_timeout(1800)
        elif repo=='satori':
            ...
            await click(frame.get_by_role('button',name='取消',exact=True).last);await shot('02')
        else:
    ...
    if state['success']:
        result=subprocess.run(['ffmpeg','-hide_banner','-loglevel','error','-y','-ss',str(max(0,start-.15)),'-i',str(path),'-an','-c:v','libx264','-preset','fast','-crf','23','-pix_fmt','yuv420p','-movflags','+faststart','-threads','2',str(folder/'demo.mp4')],capture_output=True,text=True)
