...
impl WindowsNativeSegmentEncoder {
    fn start(
        ...
        let worker = std::thread::spawn(move || -> Result<()> {
            ...
            let mut mixer =
                WindowsAudioMixer::new(system_spec.is_some(), microphone_spec.is_some());
                ...
                match input {
                    ...
                    WindowsNativeInput::Audio(index, bytes) => {
                        ...
                        if let Some(spec) = spec {
                            mixer.push(index, pcm_to_48khz_stereo_i16(&bytes, spec));
        ...
        inputs.push(std::thread::spawn(move || loop {
            ...
            match video_rx.recv_timeout(PIPE_WRITER_POLL_INTERVAL) {
                Ok(frame) => {
...
impl WindowsAudioMixer {
    ...
    fn push(&mut self, index: usize, samples: Vec<i16>) {
...
impl SegmentEncoder {
    ...
    pub fn start(
        ...
        Ok(Self {
            inner: SegmentEncoderInner::Ffmpeg(FfmpegSegmentEncoder::start(
                config, out_path, ffmpeg, video_rx, audio_rx, mic_rx,
            )?),
        })
    }
    ...
    pub fn finish(self) -> Result<PathBuf> {
        match self.inner {
            SegmentEncoderInner::Ffmpeg(encoder) => encoder.finish(),
            #[cfg(windows)]
    ...
    pub fn cancel(self) {
        match self.inner {
            SegmentEncoderInner::Ffmpeg(encoder) => encoder.cancel(),
            #[cfg(windows)]
...
mod tests {
    ...
    fn native_audio_mixer_combines_system_and_microphone() {
        let mut mixer = WindowsAudioMixer::new(true, true);
        mixer.push(0, vec![10_000; WindowsAudioMixer::BLOCK_SAMPLES]);
        mixer.push(1, vec![5_000; WindowsAudioMixer::BLOCK_SAMPLES]);
