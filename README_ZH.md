...
录屏完成后不会包含 Kiri 自己的覆盖层和控制条。

## 功能

- **截屏** — 点选窗口或框选区域,模式切换栏可随时移开;完成后自动复制到剪贴板并保存到本地素材库。
- **标注** — 画笔、图形、箭头、文字、马赛克,支持撤销/重做;已添加的标注可再次选中编辑。
- **OCR** — 默认使用本地文字识别(macOS Vision / Windows.Media.Ocr),也可同时保存阿里云、OpenAI,或其他支持图片输入并实现 OpenAI Chat Completions 接口的兼容服务。每次远程识别前都会显示目标地址、模型与当前选区的图片信息;只有明确点击「发送」或「重试」才会上传该选区。失败后不会自动重试、切换服务商或改用其他地址上传。
- **录屏** — 区域录屏,可选系统声音、麦克风、指针与点击轨迹;3-2-1 倒计时、可拖动的控制条(Esc 停止)、Retina 高清 MP4。
- **GIF** — 把短视频转成循环 GIF,转换期间持续显示进度。
