...
> **注記:** この日本語版は v1.3 の設定画面と任意のリモート OCR の説明をまだ完全には反映していません。最新のプライバシー境界は [English README](README.md) を参照してください。
...
- **OCR** — 既定ではローカル文字認識(macOS Vision / Windows.Media.Ocr)。任意で Alibaba Cloud、OpenAI、画像入力対応の OpenAI Chat Completions 互換プロファイルを複数保存できます。
...
- **macOS**:解凍して `Kiri.app` を「アプリケーション」フォルダへ移動します。グローバルショートカットには「入力監視」、キャプチャには「画面とシステム音声の録画」権限が必要です。マイク録音を有効にした場合のみ「マイク」権限も必要です。自分で書き出すか、現在の OCR 選択範囲を設定済みサービスへ明示的に送信しない限り、キャプチャは Mac 上に留まります。
