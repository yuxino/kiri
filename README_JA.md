...
矩形・矢印・文字の注釈と、取り消し・やり直し。 実際のフロントエンドをサンプルデータで操作した録画です。OS の画面撮影・OCR・書き出しの検証ではありません。
<!-- /project-demo-v1 -->
...
v1.4.9 以降は、設定から署名検証済みアップデートの確認、ダウンロード、インストールを手動で実行できます。各手順には明示的なクリックが必要で、バックグラウンド確認やサイレントインストールは行いません。v1.4.8 以前からは、まず Releases で v1.4.9 を一度手動インストールしてください。
