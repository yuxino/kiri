...
macOS Universal と Windows x64 の公開インストーラーは [GitHub Releases](https://github.com/yuxino/kiri/releases) に掲載されます。

v1.4.9 以降は、設定から署名検証済みアップデートの確認、ダウンロード、インストールを手動で実行できます。各手順には明示的なクリックが必要で、バックグラウンド確認やサイレントインストールは行いません。v1.4.8 以前からは、まず Releases で v1.4.9 以降を一度手動インストールしてください。
...
- **Windows 11 (x64)**：x64 インストーラーを提供しています。キャプチャ全体の実機検証状況は[ロードマップ](ROADMAP.md)を参照してください。`.exe` インストーラーを実行します。画面キャプチャに追加権限は不要で、マイクは Windows のプライバシー設定に従います。インストーラーは Authenticode 署名されていないため、SmartScreen が警告を表示する場合があります。
...
再編集可能なスクリーンショットは注釈前の元画像もローカルに保存するため、モザイクや図形で隠した画素が残る場合があります。トリミングを保存すると範囲外の画素も削除されます。macOS は MP4 録画、結合、サムネイル、GIF 生成に AVFoundation と ImageIO を使用し、Windows は Media Foundation とシステム画像機能を使用します。どちらのプラットフォームも FFmpeg をダウンロードせず、メディア処理は端末内で完結します。
